...
- (void)callOutShareGoodS:(LSYGoodsInfo*)goods
{
    ...
    NSString* shareURL;
    if(goods.isSeckilling)
    {
        shareURL = [NSString stringWithFormat:@"%@#/share/activity/%@/%@",imageViewUrl,goods.goods_id,PROINDEN];
    }
    else
    {
        shareURL = [NSString stringWithFormat:@"%@#/share/common/%@/%@",imageViewUrl,goods.goods_id,PROINDEN];
    }
    ...
    self.shareContent = [NSString stringWithFormat:@"%@商城里的%@，真心不错啊!快来看看吧! %@",@"芒果财经",goods.name,shareURL];
...
- (void)callOutShareViewWithUseController:(UIViewController *)addController andSharedUrl:(NSString*)url
{
    ...
    [UMSocialData defaultData].extConfig.qqData.title=@"来自芒果财经的分享";//qq
    [UMSocialData defaultData].extConfig.qzoneData.title=@"来自芒果财经的分享";//qq空
    [UMSocialData defaultData].extConfig.wechatSessionData.title=@"来自芒果财经的分享";//微信好友
    [UMSocialData defaultData].extConfig.tencentData.title=@"来自芒果财经的分享";//腾讯
    
    [UMSocialData defaultData].extConfig.sinaData.shareText = [NSString stringWithFormat:@"%@ %@",@"来自芒果财经的分享",url];//微博
    ...
    if (url == nil) {
        url = @"http://mgcj.sinosns.cn/";
    }
    //    // 友盟分享  5328fbfa56240b9ada067458
    [UMSocialData setAppKey:@"56610d70e0f55a53c100372a"];
    // 设置是否打开log输出，默认不打开，如果打开的话可以看到此sdk网络或者其他操作，有利于调试
    [UMSocialData openLog:YES];
    // 设置sdk所有页面需要支持屏幕方向
    [UMSocialConfig setSupportedInterfaceOrientations:UIInterfaceOrientationMaskAll];
    //设置微信AppId，设置分享url，默认使用友盟的网址
    [UMSocialWechatHandler setWXAppId:@"wx0bd20f42766c48ff" appSecret:@"f6a1bc932df56c4fe0036f16f6f762d5" url:url];
    //打开新浪微博的SSO开关
    [UMSocialSinaHandler openSSOWithRedirectURL:@"http://sns.whalecloud.com/sina2/callback"];
    
    //设置分享到QQ空间的应用Id，和分享url 链接
    [UMSocialQQHandler setQQWithAppId:@"1104925879" appKey:@"kdfAHKrtRukCA3fb" url:url];
    //设置支持没有客户端情况下使用SSO授权
    ...
    int btn_y = 182;
    NSArray *imageNames = @[@"videodetails_pop_btn_xinlang.png",@"videodetails_pop_btn_wixin.png",@"videodetails_pop_btn_pengyouquan.png",@"videodetails_pop_btn_qq.png",@"videodetails_pop_btn_qqkongjian.png"];
    NSArray *shareTitle = @[@"新浪微博",@"微信",@"朋友圈",@"QQ",@"QQ空间"];
    ...
    UIButton *cancleButton = [[UIButton alloc] initWithFrame:CGRectMake(0, btn_y *SP_WIDTH, SCREENWIDTH, 60 *SP_WIDTH)];
    //一行的个数
    int num = 4;
    ...
    for (int i = 0; i < 5; i ++ ) {
        ...
        UIView *buttonView = [[UIView alloc] initWithFrame:CGRectMake(buttonViewW * (temp01 % num), ( temp01  / num * buttonViewW), buttonViewW, buttonViewW)];
    ...
    cancleButton.frame = CGRectMake(0, btn_y *SP_WIDTH, SCREENWIDTH, 60 *SP_WIDTH);
