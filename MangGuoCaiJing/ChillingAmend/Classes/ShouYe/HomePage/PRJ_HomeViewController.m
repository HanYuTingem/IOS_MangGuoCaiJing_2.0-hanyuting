...
//    self.bannerBusinessModels = [[NSMutableArray alloc] init];
    ...
    [self setNavigationBarWithState:0 andIsHideLeftBtn:YES andTitle:@"芒果财经"];
...
//            }
    ...
    // 1 商品  2视频   3活动场 4仅作展示 5秒杀列表 6商品类目 7商家详情 8摇一摇 9商户列表 11捞一捞
    if ([model.activityType integerValue] == 1) {
        LSYGoodsInfoViewController *productDetail = [[LSYGoodsInfoViewController alloc] initWithNibName:@"LSYGoodsInfoViewController" bundle:nil];
        productDetail.goods_id = model.activityID;
        productDetail.isSeckilling = NO;
...
//        {
//            vc.userID = [NSString stringWithFormat:@"%@",MyObjectForKey(UserIDKey)];
//        }
//        else
//        {
//            vc.userID = @"";
...
- (void)GCRequest:(GCRequest *)aRequest Finished:(NSString *)aString
{
    ...
    ;
     NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:[aString dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
    NSLog(@"-----------------%@",dic);
    if (dic) {
...
- (void)reloadView {
    ...
    if (![[[NSUserDefaults standardUserDefaults] valueForKey:@"cityName"] isEqualToString:@"湖南省"]) {
    ...
    if ((isSmallBanner <= 3) && (isGoods == 0) && (isMerchantRequestView <=1)) {
        ...
        self.VideoView.frame = CGRectMake(ORIGIN_X(self.VideoView),ORIGIN_Y(self.HuoDongView) + CGRectGetHeight(self.HuoDongView.frame) + 10 , CGRectGetWidth(self.VideoView.frame), CGRectGetHeight(self.VideoView.frame));
...
- (void)toBusinessInfoVC:(NSString *)businessId {
    if (kkNickDicPHP == nil || [LYUserId isEqualToString:@""]) {
