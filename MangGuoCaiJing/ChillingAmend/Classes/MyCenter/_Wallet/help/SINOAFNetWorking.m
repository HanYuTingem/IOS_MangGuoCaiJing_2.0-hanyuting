...
+(void)postWithBaseURL:(NSString *)url controller:(UIViewController *)controller success:(HttpSuccess)success failure:(HttpFailure)failure noNet:(HttpNoNet)noNet
{
    ...
    [GCUtil connectedToNetwork:^(NSString *connectedToNet) {
        ...
        NSLog(@"connectedToNet == --- %@",connectedToNet);
    }];
}
...
+ (void)requestPostWithBaseURL:(NSString *)url params:(NSDictionary *)params success:(HttpSuccess)success failure:(HttpFailure)failure {
    ...
    AFHTTPRequestOperationManager* manager=[AFHTTPRequestOperationManager manager];
    //创建字典
//    manager.requestSerializer = [AFJSONRequestSerializer serializer];//请求格式json
...
#pragma  mark - 优惠券请求添加了请求格式为json
+(void)postCouponWithBaseURL:(NSString *)url controller:(UIViewController *)controller params:(NSDictionary *)params success:(HttpSuccess)success failure:(HttpFailure)failure noNet:(HttpNoNet)noNet{
    [GCUtil connectedToNetwork:^(NSString *connectedToNet) {
        if ([connectedToNet isEqualToString:NotReachable]) {
            [MBProgressHUD showConnectNetWork:connectedToNet toView:controller.view];
            noNet();
            ...
            [self requestCouponPostWithBaseURL:url params:params success:success failure:failure ];
        }
    }];
}

+ (void)requestCouponPostWithBaseURL:(NSString *)url params:(NSDictionary *)params success:(HttpSuccess)success failure:(HttpFailure)failure {
