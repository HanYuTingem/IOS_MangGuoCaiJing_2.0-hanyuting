...
+ (BOOL)netWorkIsOk{
    ...
    if(networkReachabilityStatus == AFNetworkReachabilityStatusReachableViaWWAN || networkReachabilityStatus == AFNetworkReachabilityStatusReachableViaWiFi){
        return YES;
    }else if(networkReachabilityStatus == AFNetworkReachabilityStatusUnknown || networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable){
        return NO;
    }else{
        return NO;
    }
...
//     12         */
//    NSString * CM = @"^1(83|78|34[0-8]|(3[5-9]|5[017-9]|8[278]|78)\\d)\\d{7}$";
//    /**
...
//    
//    NSString *CT_NUM = @"^((133)|(153)|(177)|(18[0,1,9])|(14[0-9]))\\d{8}$";
//    
//    NSPredicate *regextestmobile = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", MOBILE];
//    NSPredicate *regextestcm = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CM];
//    NSPredicate *regextestcu = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CU];
//    NSPredicate *regextestct = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CT];
//    NSPredicate *regextestct1 = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CT_NUM];
//    
//    
...
//    hud.yOffset = kkViewHeight/2;
//    [MBProgressHUD showHUDAddedTo:self animated:YES].center = self.center;
}
...
+ (void)removeDumplingInforOfFailure{
    
    ZHLog(@"%d,%d",[MyObjectForKey(DumplingTheLastTimeKey) intValue],[[LYLTools currentDateWithDay]intValue]);
    
    //当前时间和过去比较看是否失效
    if([MyObjectForKey(DumplingTheLastTimeKey) intValue] != [[LYLTools currentDateWithDay]intValue]){
        //饺子失效 清除缓存
        if([[ZHDataBase sharedDataBase]deleWithLogingState:@"0"]){//清除未登录
            ZHLog(@"清除未登录饺子成功");
        }else{
            ZHLog(@"清除未登录饺子失败");
        }
        
        if([[ZHDataBase sharedDataBase]deleWithLogingState:@"1"]){//清除已登录
            ZHLog(@"清除登录饺子成功");
...
+ (NSMutableArray *)noGetMeoneyWithReturnDumplingId{
    NSMutableArray *dumplingInforNoLogingArray = [[ZHDataBase sharedDataBase]selectLogingStateWithReturnDumplingInfor:@"0" andUserId:@""];
    NSMutableArray *dumplingIdArray = [NSMutableArray array];//未登录没有领取的饺子钱
    for (DumplingInforModel *model in dumplingInforNoLogingArray) {
        [dumplingIdArray addObject:model.resultListModel.dumplingModel.prizeId];
    }
    return dumplingIdArray;
}
...
+ (void)setViewWithBorder:(UIView *)objectView cornerRadius:(CGFloat)connerRadius borderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth{
    objectView.layer.masksToBounds = YES;
    objectView.layer.cornerRadius = connerRadius;
    objectView.layer.borderColor = borderColor.CGColor;
    objectView.layer.borderWidth = borderWidth;
}

+ (NSAttributedString *)attributeWithStr:(NSString *)str rangeStr:(NSString *)rangeStr rangColor:(UIColor *)rangColor{
    NSRange range = [str rangeOfString:rangeStr];
    NSMutableAttributedString *attributeStr=  [[NSMutableAttributedString alloc]initWithString:str];
    [attributeStr addAttribute:NSForegroundColorAttributeName value:rangColor range:range];
    return attributeStr;
}
@end
