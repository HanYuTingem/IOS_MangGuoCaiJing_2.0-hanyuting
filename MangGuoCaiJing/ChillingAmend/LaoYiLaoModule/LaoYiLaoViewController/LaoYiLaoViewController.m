...
@interface LaoYiLaoViewController ()<LoadEffectViewDelegate,UIScrollViewDelegate,AnimateViewDelegate,SSNoNetViewDelegate,SSInfoDumpViewDelegate,GetMoneyViewDelegate,SSBaoJiaoZiBtnViewDelegate>
{
    ...
    UILabel  * _noDataLabel;//没有网络时的提示label
...
- (void)viewDidLoad {
    ...
    if(IsStrWithNUll(_userID)){
        MySetObjectForKey(LoginStateNO, LoginStateKey);//设置没有登录
    }else{
    ...
    //    MySetObjectForKey(LoginStateNO, LoginStateKey);//设置已经登
    //    MyObjectForKey(key)
...
#pragma mark SSDumplingDelegate
...
- (void)woDeJiaoZiView{
    ...
    _noDataLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, kkViewWidth, info_H-PosterHeight)];
    _noDataLabel.text = @"没有网络,请检查网络连接";
...
- (void)laoYiLaoAnimateFinished{
    //    [_bounceView addSharedWithLogingView];
    //    [_bounceView addSharedWithCeilingView];
    
    [_bounceView addDumplingInforView];
    [[UIApplication sharedApplication].keyWindow addSubview:_bounceView];
    
    NSLog(@"结束捞");
    //    _animateView.dumplingsAndSpoonView.dumplingsBtn.hidden = YES;
}
...
- (void)loadDataNoLoginStatusMaxNumber{
    ...
    //            [self networkStatusOKWithView];
                [self loadDataUserDumplingList];
                //            _remainingNumber = _maxNumber;//剩余次数为
            }
        }
    ...
    [LYLAFNetWorking getWithBaseURL:[LYLHttpTool noLogingUserDumplingWithProductCode:ProductCode sysType:SysType andSessionValue:SessionValue] success:^(id json) {
        ...
        if([((NSDictionary *)json)[@"code"] isEqualToString:@"100"]){
            ...
            if(dumplingInforArray.count < _maxNumber){
                ...
                //            [dumplingIdArray addObject:dumplingModel.resultListModel.dumplingModel];
...
- (void)loadDataLogingDumping{
    ...
    [LYLAFNetWorking getWithBaseURL:[LYLHttpTool logingUserDumplingWithUserId:MyObjectForKey(UserIDKey) productCode:ProductCode sysType:SysType andSessionValue:SessionValue] success:^(id json) {
        ZHLog(@"登陆用户捞饺子json =%@",json);
        [self hideHud];
        
        if([((NSDictionary *)json)[@"code"] isEqualToString:@"102"]){//没有次数
            [_bounceView addSharedWithCeilingView];
            [[UIApplication sharedApplication].keyWindow addSubview:_bounceView];//没次数
            ...
            /**
             处理空对象，以为有空对象writeToFile会失败
             */
            //            NSMutableDictionary * jsonDic = [[NSMutableDictionary alloc]initWithDictionary:json];
            //            NSMutableDictionary *resultDic = [[NSMutableDictionary alloc]initWithDictionary:jsonDic[@"resultList"]];
            //            NSMutableDictionary *dumplingDic = [[NSMutableDictionary alloc]initWithDictionary:resultDic[@"dumpling"]];
            //
            //            NSArray *arr = [dumplingDic allKeysForObject:[NSNull null]];
            //
            //            [arr enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL *stop) {
            //
            //                [dumplingDic setObject:@"" forKey:arr[idx]];
            //            }];
            //            [resultDic setObject:dumplingDic forKey:@"dumpling"];
            //            [jsonDic setObject:resultDic forKey:@"resultList"];
            //            ZHLog(@"登陆用户捞饺子jsonDic =%@",jsonDic);
...
- (void)loadDataMyDumpling{
    ...
    NSString * urlStr = [LYLHttpTool myDumplingWithProductCode:ProductCode sysType:SysType andSessionValue:SessionValue UserId:MyObjectForKey(UserIDKey)];
...
- (void)loadDataRemainingDumplingWithNumber{
    ...
    [LYLAFNetWorking getWithBaseURL:[LYLHttpTool currentDumplingWithNumberProductCode:ProductCode sysType:SysType andSessionValue:SessionValue]success:^(id json) {
        ...
        if([json[@"code"] isEqualToString:@"100"]){
            ...
            [_animateView startTimer];
            [self networkStatusOKWithView];
            if(![LogingState isEqualToString:LoginStateYES]){//未登录
